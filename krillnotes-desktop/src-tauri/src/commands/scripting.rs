...
pub fn get_schema_fields(
    ...
    let schema_def = workspace.script_registry().get_schema(&schema)
        .map_err(|e: crate::KrillnotesError| { log::error!("get_schema_fields failed: {e}"); e.to_string() })?;
...
pub fn invoke_tree_action(
    ...
    workspace.run_tree_action(&note_id, &label)
        .map_err(|e| { log::error!("invoke_tree_action failed: {e}"); e.to_string() })
}
...
pub fn get_note_view(
    ...
    let workspaces = state.workspaces.lock().expect("Mutex poisoned");
    let workspace = workspaces.get(label).ok_or("No workspace open")?;
    workspace.run_view_hook(&note_id).map_err(|e| { log::error!("get_note_view failed: {e}"); e.to_string() })
}
...
pub fn get_note_hover(
    ...
    let workspaces = state.workspaces.lock().expect("Mutex poisoned");
    let workspace = workspaces.get(label).ok_or("No workspace open")?;
    workspace.run_hover_hook(&note_id).map_err(|e| { log::error!("get_note_hover failed: {e}"); e.to_string() })
}
...
pub fn render_view(
    ...
    let workspaces = state.workspaces.lock().expect("Mutex poisoned");
    let workspace = workspaces.get(label).ok_or("No workspace open")?;
    workspace.render_view(&note_id, &view_label).map_err(|e| { log::error!("render_view failed: {e}"); e.to_string() })
}
