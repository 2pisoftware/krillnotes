# Identity Model Foundation — Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Implement the cryptographic identity foundation in `krillnotes-core` — keypair generation, Argon2id passphrase protection, identity file I/O, and a settings registry that binds workspaces to identities with encrypted DB passwords.

**Architecture:** A single `identity.rs` module in `krillnotes-core/src/core/` with an `IdentityManager` struct that owns the config directory path. It manages identity files (`~/.config/krillnotes/identities/<uuid>.json`) and its own settings file (`identity_settings.json`). Crypto chain: passphrase → Argon2id → AES-256-GCM → Ed25519 seed → HKDF → DB password encryption.

**Tech Stack:** `ed25519-dalek` 2.x, `argon2` 0.5, `aes-gcm` 0.10, existing `hkdf`/`sha2`/`rand`/`base64`

**Design doc:** `docs/plans/2026-03-04-identity-model-design.md`
...
**Files:**
- Modify: `krillnotes-core/Cargo.toml:9-29`

**Step 1: Add new dependencies**

Add after line 29 (the existing crypto deps block):
...
git commit -m "chore: add ed25519-dalek, argon2, aes-gcm dependencies for identity model"
```
...
**Files:**
- Modify: `krillnotes-core/src/core/error.rs:6-59` (KrillnotesError enum)
- Modify: `krillnotes-core/src/core/error.rs:90-112` (user_message impl)

**Step 1: Add identity error variants**

Add these variants to the `KrillnotesError` enum (before the closing `}`):
...
**Step 2: Add user_message arms**

In the `user_message()` method, add arms for each new variant:
...
Run: `cargo check -p krillnotes-core`
Expected: compiles with no errors (new variants are unused warnings — fine)

**Step 4: Commit**

```bash
git add krillnotes-core/src/core/error.rs
git commit -m "feat: add identity error variants to KrillnotesError"
```

---

### Task 3: Create identity.rs — Serde Types & Module Wiring

**Files:**
- Create: `krillnotes-core/src/core/identity.rs`
- Modify: `krillnotes-core/src/core/mod.rs:6-18` (add module declaration)
- Modify: `krillnotes-core/src/lib.rs:11-30` (add re-export)

**Step 1: Write the failing test**

Create `krillnotes-core/src/core/identity.rs` with just the types and a test:
...
In `krillnotes-core/src/core/mod.rs`, add after the existing module declarations (around line 18):
...
In `krillnotes-core/src/lib.rs`, add a re-export in the existing block (around line 30):

```rust
#[doc(inline)]
pub use core::identity::{IdentityFile, IdentityManager, IdentitySettings, IdentityRef, WorkspaceBinding, UnlockedIdentity};
```

(Note: `UnlockedIdentity` doesn't exist yet — add it in Task 4. For now, omit it from the re-export and add it later.)

For now, re-export only what exists:

```rust
#[doc(inline)]
pub use core::identity::{IdentityFile, IdentityManager, IdentitySettings, IdentityRef, WorkspaceBinding};
```

**Step 4: Run tests**

Run: `cargo test -p krillnotes-core -- identity`
Expected: all 4 tests pass

**Step 5: Commit**

```bash
git add krillnotes-core/src/core/identity.rs krillnotes-core/src/core/mod.rs krillnotes-core/src/lib.rs
git commit -m "feat: add identity module with serde types and IdentityManager scaffold"
```

---

### Task 4: Implement create_identity
...
```rust
    ...
    fn test_create_identity() {
        ...
        let pk_bytes = base64::engine::general_purpose::STANDARD
            .decode(&identity_file.public_key).unwrap();
...
Add `use base64::Engine;` at the top of the tests module.

**Step 2: Run test to verify it fails**

Run: `cargo test -p krillnotes-core -- test_create_identity`
Expected: FAIL — `create_identity` method does not exist

**Step 3: Implement create_identity**

Add these imports to the top of `identity.rs`:
...
Add the `UnlockedIdentity` struct after `IdentityManager`:
...
Add the method to `impl IdentityManager`:
...
git commit -m "feat: implement create_identity with Ed25519 keygen and Argon2id encryption"
```

---

### Task 5: Implement unlock_identity
...
Run: `cargo test -p krillnotes-core -- test_unlock_identity test_wrong_passphrase test_sign_and_verify`
Expected: FAIL — `unlock_identity` does not exist

**Step 3: Implement unlock_identity**

Add to `impl IdentityManager`:
...
Add `use ed25519_dalek::Signer;` to the imports at the top of the file (needed for `.sign()` in tests).
...
```bash
git add krillnotes-core/src/core/identity.rs
git commit -m "feat: implement unlock_identity with Argon2id decryption"
```

---

### Task 6: Implement list_identities and delete_identity
...
Run: `cargo test -p krillnotes-core -- test_list_identities test_delete_identity test_delete_identity_with_bound`
...
Run: `cargo test -p krillnotes-core -- test_list_identities test_delete_identity`
Expected: all 3 PASS (the bound-workspaces test depends on `bind_workspace` from Task 8 — skip it for now, it will pass after Task 8)

Run the two that don't depend on bind: `cargo test -p krillnotes-core -- test_list_identities test_delete_identity --exact` (use the test names without the bound one)

Actually, the `test_delete_identity_with_bound_workspaces` test calls `bind_workspace` which doesn't exist yet. **Move that test to Task 8.** Only include the first two tests in this task.

**Step 5: Commit**

```bash
git add krillnotes-core/src/core/identity.rs
git commit -m "feat: implement list_identities and delete_identity"
```

---

### Task 7: Implement change_passphrase
...
Run: `cargo test -p krillnotes-core -- test_change_passphrase`
Expected: FAIL — `change_passphrase` does not exist
...
```bash
git add krillnotes-core/src/core/identity.rs
git commit -m "feat: implement change_passphrase for identity"
```

---

### Task 8: Implement DB Password Bind/Unbind/Decrypt
...
Also add the `test_delete_identity_with_bound_workspaces` test from Task 6 here since it depends on `bind_workspace`.

**Step 2: Run tests to verify they fail**

Run: `cargo test -p krillnotes-core -- test_bind_workspace test_db_password test_unbind test_multiple_identities_isolation test_get_workspaces`
Expected: FAIL — methods do not exist

**Step 3: Implement bind/unbind/decrypt/get_workspaces**
...
```rust
    ...
    pub fn bind_workspace(
        ...
        // HKDF: derive a per-workspace key from the seed
    ...
    pub fn decrypt_db_password(
        ...
        // HKDF: derive the same per-workspace key
...
```bash
git add krillnotes-core/src/core/identity.rs
git commit -m "feat: implement workspace bind/unbind/decrypt for identity DB passwords"
```

---

### Task 9: Add UnlockedIdentity re-export & Identity File Format Test

**Files:**
- Modify: `krillnotes-core/src/lib.rs:11-30` (add `UnlockedIdentity` to re-exports)
- Modify: `krillnotes-core/src/core/identity.rs` (add file format validation test)

**Step 1: Update lib.rs re-export**

Add `UnlockedIdentity` to the existing re-export line:
...
**Step 2: Add identity file format test**
...
Expected: all existing tests + new identity tests PASS

**Step 5: Commit**

```bash
git add krillnotes-core/src/lib.rs krillnotes-core/src/core/identity.rs
git commit -m "feat: add UnlockedIdentity re-export and identity file format validation test"
```
...
Run: `cargo test -p krillnotes-core`
Expected: all tests PASS (existing + ~15 new identity tests)

**Step 2: Check for compiler warnings**

Run: `cargo clippy -p krillnotes-core -- -W clippy::all`
Expected: no warnings (or only pre-existing ones)

**Step 3: Verify TypeScript build is unaffected**

Run: `cd krillnotes-desktop && npx tsc --noEmit`
Expected: no errors (this PR doesn't touch TypeScript)

**Step 4: Final commit if any clippy fixes were needed**

```bash
git add -A
git commit -m "chore: clippy fixes for identity module"
```
